/*
  # Add National Number Unique Constraint

  1. Changes
    - Add unique constraint on national_number column if it doesn't already exist
...
BEGIN
  ...
  IF NOT EXISTS (
    SELECT 1 
    FROM pg_constraint 
    WHERE conname = 'casino_players_national_number_key'
  ) THEN
